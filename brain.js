...
if (canvas) {
  ...
  if (!window.THREE || !hasWebGLSupport) {
    ...
    const baseColor = new THREE.Color('#f2f6ff');
    const highlightColor = new THREE.Color('#ffffff');
    const hemisphereAccent = new THREE.Color('#ffd966');
    const pointerColor = new THREE.Color('#9fd8ff');
    let hemisphereHighlight = 'none';
    ...
    function updateColors(time) {
      ...
      const accentR = hemisphereAccent.r;
      const accentG = hemisphereAccent.g;
      const accentB = hemisphereAccent.b;

      const highlightLeft = hemisphereHighlight === 'left';
      const highlightRight = hemisphereHighlight === 'right';
      ...
      for (let i = 0; i < POINT_COUNT; i++) {
        ...
        if ((highlightLeft && baseX <= 0) || (highlightRight && baseX >= 0)) {
          targetR = accentR;
          targetG = accentG;
          targetB = accentB;
        }

        const r = baseR + (targetR - baseR) * brightness;
        const g = baseG + (targetG - baseG) * brightness;
        const b = baseB + (targetB - baseB) * brightness;
    ...
    window.setBrainHighlight = mode => {
      hemisphereHighlight = mode === 'left' || mode === 'right' ? mode : 'none';
    };

    if (window.pendingBrainHighlight) {
      window.setBrainHighlight(window.pendingBrainHighlight);
...
function startFallbackBrain(canvas) {
  ...
  for (let i = 0; i < NODE_COUNT; i++) {
    ...
    nodes.push({
      ...
      x: 0,
      y: 0,
      hemisphere: point.x <= 0 ? 'left' : 'right',
    });
  }
  ...
  const highlightState = { mode: 'none' };
    ...
    for (let i = 0; i < NODE_COUNT; i++) {
      ...
      for (let j = i + 1; j < NODE_COUNT; j++) {
        ...
        const highlight =
          ...
          (nodeA.hemisphere === highlightState.mode ||
            nodeB.hemisphere === highlightState.mode);
        if (highlight) {
          ctx.strokeStyle = `rgba(255, 217, 102, ${alpha})`;
        } else {
  ...
  function drawNodes() {
    ...
    for (const node of nodes) {
      ctx.beginPath();
      if (highlightState.mode !== 'none' && node.hemisphere === highlightState.mode) {
  ...
  window.setBrainHighlight = mode => {
    highlightState.mode = mode === 'left' || mode === 'right' ? mode : 'none';
  };
